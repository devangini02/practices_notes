...
#You'll need to uncomment this if you're running Keras for the first time:
...
setwd("C:/Users/vanal/OneDrive/Desktop/Uni Work/Machine Learning/Homework/Keras Tutorial")

#Let's sed the seed for the random number generator
set.seed(2)

df<-read.csv("welfarelabel.csv") #Read in the data as a dataframe
...
df <- df %>% dplyr::select(one_of(all_variables_names)) #Drop all other variables
...
model %>% # Adds a densely-connected layer with 16 units to the model:
  keras::layer_dense(units = 16, activation = 'relu',kernel_initializer=keras::initializer_random_uniform(minval = -0.001, maxval = 0.001, seed = 2)) %>%
  ...
  #hidden layer. 
  keras::layer_dense(units = 1, activation = 'sigmoid',kernel_initializer=keras::initializer_random_uniform(minval = -0.001, maxval = 0.001, seed = 2))
...
model %>% keras::compile(
  optimizer = keras::optimizer_adagrad(learning_rate=0.01), #We'll optimize using the adagrad algorithm discussed in class
  loss = 'mse'
)
...
model %>% keras::fit(
  ...
  epochs = 200,    #This controls how many times the optimizer goes through the whole dataset before it finishes training
